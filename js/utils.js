...
export async function fetchBundle(bundleUrl) {
  console.warn(`[MOCK] fetchBundle: ссылка ${bundleUrl} неактивна. Возвращаю фейковый bundle`);

  // Минимальный фейковый бинарный blob (например, просто "Unity")
  const dummyData = new Uint8Array([0x55, 0x6e, 0x69, 0x74, 0x79]); // ASCII: U n i t y
